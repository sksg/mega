...
def parallize(signature, otypes=None, doc=None, default='sequential',
              evn='MEGA_PARALLIZE', isvec=False, parallel='threads',
              passinfo=False, reducefunc=None):
    def wrap_parallized(pyfunc):
        return parallized(pyfunc, signature, otypes, doc, default,
                          evn, isvec, parallel, passinfo, reducefunc)
...
class parallized(object):  # inspired by np.vectorize
    def __init__(self, pyfunc, signature, otypes=None, doc=None,
                 ...
                 parallel_type='threads', passinfo=False, reducefunc=None):
        ...
        self.parameters = [k for k in parameters]
        empty = inspect.Parameter.empty
        if self.passinfo:
            self.parameters = self.parameters[1:]
        self.mandatory_count = len([k for k in self.parameters
                                    if parameters[k].default is empty])
    ...
    def _process_args(self, args, kwargs):
        ...
        for p in self.parameters:
            if p in kwargs:
                ...
                if len(args) == 0:
                    if len(allargs) < self.mandatory_count:
                        msg = 'expected at least {} and got {}'
                        msg = msg.format(self.mandatory_count, len(givenargs))
        ...
        if len(args) != 0:
            msg = 'expected max {} and got {}'.format(len(self.parameters),
                                                      len(givenargs))
    ...
    def sequential(self, *args, **kwargs):
        ...
        for index in np.ndindex(*broadcast_shape):
            ...
            if self.passinfo:
                pinfo = parallelinfo(broadcast_shape, index)
                results = self.pyfunc(pinfo, *i_args)
            else:
        ...
        if len(broadcast_shape) == 0:
            outputs = tuple(np.asscalar(o)
                            if isinstance(o, np.ndarray) and o.size == 1 else o
                            for o in outputs)
        if self.reducefunc is not None:
            return self.reducefunc(*outputs)
    ...
    def parallel(self, *args, **kwargs):
        ...
        for index in np.ndindex(*broadcast_shape):
            ...
            if self.passinfo:
                pinfo = parallelinfo(broadcast_shape, index)
                futures[pool.submit(self.pyfunc, pinfo, *i_args)] = index
            else:
...
class asparallel(object):
    def __init__(self, pyfunc, default='sequential', evn='MEGA_PARALLIZE'):
        self.pyfunc = pyfunc
        self.default = default
        self.evn = evn
        self.__wrapped__ = pyfunc
